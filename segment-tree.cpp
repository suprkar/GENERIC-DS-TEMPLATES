...
/*
  ...
  In a given data set of size 'n', a segment tree can be utilised to process queries in a range [l,r]. These queries may require to process a function f,
  taking input in the given range.
  
  Also, the segment tree can be used to update a value in the data set as well. Additionally, we can perform something known as Lazy Propagation to update
  values in a given range as well.
  
  The time-complexities for the given operations are discussed below.
  
  Range Queries: O(log n)
  Point Update: O(log n)
  Range Update(Lazy Propagation): O(log n)
  
  Example: 1. Finding the sum of range [100,203] in the given data set.
           2. Finding the median in the range [1002, 3943].
*/
